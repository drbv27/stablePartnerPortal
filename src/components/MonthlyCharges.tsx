      ...
      <table className="w-full border border-gray-500">
        <thead>
          <tr>
            <th
              colSpan={6}
              className="px-1 md:px-6 py-1  border border-gray-300 text-left text-gray-500  tracking-wider"
            >
          ...
          <tr>
            <th
              colSpan={6}
              className="px-1 md:px-6 py-1  border border-gray-300 text-center text-base text-gray-500 tracking-wider bg-orange-200"
            >
              Monthly Charges
            </th>
          </tr>
        ...
        <tbody>
          <tr>
            <td
              colSpan={3}
              className="px-1 md:px-6 py-1 whitespace-no-wrap border border-gray-300 leading-5 text-sm md:text-base"
            >
              Description
            </td>
            <td
              colSpan={1}
              className="px-1 md:px-2 py-1 whitespace-no-wrap border border-gray-300 leading-5 text-center text-sm md:text-base"
            >
              Qty
            </td>
            <td
              colSpan={1}
              className="px-1 md:px-6 py-1 whitespace-no-wrap border border-gray-300 leading-5 text-center text-sm md:text-base"
            >
              Rate
            </td>
            <td
          ...
          {Users.total !== 0 && (
            <tr>
              ...
              >
                <span>$</span>
                <span className="md:ms-1">{Users.price.toFixed(2)}</span>
              </td>
              <td
                colSpan={1}
          ...
          {Conference.total !== 0 && (
            <tr>
              ...
              >
                <span>$</span>
                <span className="md:ms-1">{Conference.price.toFixed(2)}</span>
              </td>
              <td
                colSpan={1}
                className="font-semibold px-1 md:px-6 py-1 border border-gray-300 leading-5 text-right text-sm md:text-base"
            ...
            <tr>
              ...
              >
                ...
                {portNumbers.length - 2 > 0
                  ? ((portNumbers.length - 2) * 2).toFixed(2)
                  : 0}
              </td>
            </tr>
          )}
          ...
          {totalProducts
            .filter((product) => product.recurrent)
            .map((product) => (
              <tr key={product.id}>
                <td
                  colSpan={3}
                ...
                >
                  <span>$</span>
                  <span className="md:ms-1">{product.price.toFixed(2)}</span>
                </td>
                <td
                  colSpan={1}
                  className="font-semibold px-1 md:px-6 py-1 border border-gray-300 leading-5 text-right text-sm md:text-base"
          ...
          {totalEntrieProducts
            ...
            .map((product) => (
              <tr key={product.id}>
                <td
                  colSpan={3}
                  className="md:font-semibold px-1 md:px-6 py-1 border border-gray-300 leading-5 text-sm md:text-base"
                >
                  {product.title} |{" "}
                  <small className="font-normal"> {product.description}</small>
                </td>
          ...
          <tr>
            <td
              colSpan={5}
              className="px-1 md:px-6 py-1 border border-gray-300 bg-orange-200 leading-5"
            >
              <span className="font-semibold">Total Monthly Charges</span>
              <span className="text-xs">
                {" "}
                (Prices do not include taxes or local government fees. Taxes and
                fees are calculated based on location of first billing
                statement)
              </span>
            </td>
            <td
              colSpan={1}
              className="font-semibold px-1 md:px-6 py-1 border border-gray-300 bg-orange-200 leading-5"
            >
