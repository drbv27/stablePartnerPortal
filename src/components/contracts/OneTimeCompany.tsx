...
const OneTimeCompany = ({quote}:any) => {
    ...
    if (totalProducts.length !== 0) {
        monthlyTotal = totalProducts.reduce((acc:number, product:any) => product.recurrent ? acc + product.total * product.price : acc, 0)
        oneTimeTotal = totalProducts.reduce((acc:number, product:any) => !product.product.recurrent ? acc + product.quantity * product.product.price : acc, 0)
        oneTimeTax = totalProducts.reduce((acc:number, product:any) => (!product.recurrent && (product as any).taxes) ? acc + product.total * product.price * TAX_RATE : acc, 0)
    }
    if (totalEntrieProducts.length !== 0) {
        ...
        oneTimeTax += totalEntrieProducts.reduce((acc:number, product:any) => (!product.recurrent && product.taxes) ? acc + product.quantity * product.price * TAX_RATE : acc, 0)
    }
            ...
            {totalProducts.filter((product:any) => !product.product.recurrent).map((product:any) => (
                <tr key={product._id}>
                    <td colSpan={4} className="font-semibold px-1 md:px-6 py-1 border border-gray-300 leading-5">
                        {product.product.title} | <small className="font-normal"> {product.description}</small>
                    </td>
                    ...
                    <td colSpan={1} className="font-semibold px-1 md:px-6 py-1 border border-gray-300 leading-5 text-right">
                        <span>$</span><span className="md:ms-1">{(product as any).taxes ? (product.quantity * product.product.price * TAX_RATE).toFixed(2) : '0.00'}</span>
                    </td>
                </tr>
            ))}
